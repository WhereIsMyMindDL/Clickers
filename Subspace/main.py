    ...
    if index_proxy <= count_acc - 1:
        ...
        try:
            ...
            browser.find_element(
                By.XPATH, "/html/body/div[9]/div/div/div/div[1]/a"
            ).click()
            time.sleep(3)
            email_input = browser.find_element(
                By.XPATH, "/html/body/div[3]/div/div/div/div/div/form/input[1]"
            )
            email_input.send_keys(take_mail())
            time.sleep(7)
            print(f"Почта №{index_proxy} успешно зарегистрирована")
            browser.quit()
            main()
